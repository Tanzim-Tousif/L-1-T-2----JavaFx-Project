...
public class SpecificRestaurantFoodName implements Initializable {
        ...
        for(int i = 0;i<restaurants.size();i++){
            if(restaurants.get(i).getName().equalsIgnoreCase(restaurantName)){
                ...
                for(int j = 0;j<foods.size();j++){
                    if(foods.get(j).getName().toLowerCase().contains(foodName.toLowerCase()) && foods.get(j).getRestaurantId() == tempid){
                        //flag = 1;
